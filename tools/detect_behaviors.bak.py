...
def main() -> None:
    ...
    # EMA alphas
    # - alpha_global applies to most behaviors
    # - alpha_turn can be set separately for head_turn behaviors
    ...
    for r in rows:
        ...
        # NOTE about "face" flags:
        # - face_detected: FaceMesh extractor ran and returned landmarks (may still be too tiny to trust)
        # - face_proxy_detected: we have a *usable* face proxy (size/pitch/yaw passed basic sanity checks)
        # For CCTV, treat face_proxy_detected as the primary "can we trust face signals" gate.
        ...
        # FaceMesh usable only when both flags are True.
        # (pose-based face proxy can set face_proxy_detected True, but face_detected will be False)
        face_mesh_usable = bool(face_mesh_ok and face_proxy_ok)

        # Face/Proxy size:
        # - For FaceMesh: face_detected=True + face_proxy_detected=True
        # - For pose proxy: face_detected=False but face_proxy_detected=True (we still have a usable size)
        face_sz = _get_optional_float(r, "face_size_px") if face_proxy_ok else None

        # Look down:
        # - Prefer FaceMesh (look_down_score)
        # - If face missing, fallback to pose proxy (look_down_pose_score)
        ...
        # Head turns:
        # IMPORTANT: only trust FaceMesh for left/right in far CCTV.
        # Extra gate: require a minimum face size (reduces false positives).
        ...
        if (not can_turn) and bool(args.allow_pose_turn):
            # Pose proxy: face_proxy_detected may be True even when face_detected is False.
            # In that case, we still rely on face_size_px computed from pose eyes.
            can_turn = bool(
                pose_ok
                and face_proxy_ok
        ...
        left_val = _get_optional_float(r, "head_turn_left_score") if can_turn else None
        right_val = _get_optional_float(r, "head_turn_right_score") if can_turn else None
        # Brief dropout bridging: FaceMesh often drops exactly during strong turns.
        # For a short window, reuse last known turn scores to avoid missing the event entirely.
        ...
        # Hand-to-face:
        # Can work with pose only (we compute a pose proxy when face is missing).
        ...
        # If one direction is already active, freeze the other direction until it ends.
        ...
        # Update behaviors (using smoothed scores)
        updates = [
